...
(async () => {
	...
	const swaps = [
		{
			...
			assetInIndex: 0,
			assetOutIndex: 1,
			amount: ethers.utils.parseUnits("1", 18), // Maximum test input
			userData: "0x",
		},
		// {
	...
	const limits = [
		BigNumber.from(10 ^ 18), // Maximum WETH input
		BigNumber.from((-1 * 10) ^ 22), // Maximum DAI movement
		// BigNumber.from((-1 * 10) ^ 18), // Large negative USDC output limit
	];
