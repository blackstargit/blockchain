...
export class Balancer implements Uniswapish {
	...
	public static getInstance(chain: string, network: string): Balancer {
		...
		if (!(chain + network in Balancer._instances)) {
			Balancer._instances[chain + network] =
				new Balancer(chain, network);
		}
	...
	public getAllowedSlippage(allowedSlippageStr?: string): number {
		if (
			allowedSlippageStr != null &&
			isFractionString(allowedSlippageStr)
		) {
			const fractionSplit =
				allowedSlippageStr.split("/");
			return Number(
				(
					(Number(fractionSplit[0]) /
						Number(
							fractionSplit[1]
						)) *
					100
	...
	async estimateBuyTrade(
		...
		const info = await this.balancer.swaps.findRouteGivenOut({
			...
			gasPrice: BigNumber.from(
				this._chain.gasPrice.toFixed(0)
			),
		});
		...
		return {
			trade: {
				swap: {
					swapInfo: info,
					maxSlippage: this.getAllowedSlippage(
						allowedSlippage
					),
