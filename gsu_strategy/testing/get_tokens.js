...
async function duplicateTransaction(numberOfTx) {
	// Fetch the original transaction details
	const accounts = await provider.listAccounts();
	if (accounts.length < numberOfTx) {
		console.error(
			"Insufficient accounts to duplicate transaction"
		);
		return;
	}
	...
	for (const [i, account] of accounts.entries()) {
		try {
			...
			duplicateTx.from = account.address;
...
(async () => await duplicateTransaction(1))();
