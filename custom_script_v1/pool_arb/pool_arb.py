...
class ArbitrageBalancer(ScriptStrategyBase):
    ...
    def on_stop(self):
        if self.current_arbitrage_trade:
            self.current_arbitrage_trade.cancel()
    ...
    async def determine_and_execute_trade(self, params: TradeParams):
        ...
        if trade_type:
            ...
            self.current_arbitrage_trade = safe_ensure_future(
                self.execute_trade(
                    params=params,
    ...
    def cancel_all_orders(self):
        ...
        for order in self.get_active_orders(connector_name=self.connector):
            self.cancel(self.connector, order.trading_pair, order.client_order_id)
